...
def check_app_status(root):
    ...
    """
    try:
        url = "https://app-control-6e8a6-default-rtdb.europe-west1.firebasedatabase.app/apps/gm_extractor.json"
...
def extract_products(html_text):
    ...
    items = soup.find_all(
        lambda tag: tag.name in ("form", "div")
        and "product-item" in (tag.get("class") or [])
    )
    ...
    for form in items:
        ...
        title_tag = form.find(
            "a",
            class_=lambda c: c and "product-item-link" in str(c)
        )
        ...
        manufacturer_spans = form.find_all(
            "span",
            class_="manufacturer-name"
        )

        if len(manufacturer_spans) >= 2:
            # First span is always the brand name,
            # remaining spans form the SKU
            brand = manufacturer_spans[0].get_text(" ", strip=True)
            sku_parts = [
                s.get_text(" ", strip=True)
                for s in manufacturer_spans[1:]
                if s.get_text(" ", strip=True)
            ]
            sku = " ".join(sku_parts)
        elif len(manufacturer_spans) == 1:
            brand = manufacturer_spans[0].get_text(" ", strip=True)
        else:
            brand = ""

        # Backup SKU from JSON block
        ...
        availability_div = form.find(
            "div",
            class_=lambda c: c and "desc" in str(c)
        )

        if availability_div:
            availability = availability_div.get_text(
                " ",
                strip=True
            )
...
class ExtractorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("GM Product Extractor")
    ...
    def show_instructions(self):
        messagebox.showinfo(
            "Instructions",
            "1. Go to the relevant GM page.\n"
            "2. Press Ctrl+U, then on the opened page press Ctrl+A then Ctrl+C.\n"
            "3. Paste into a text file named after the page, e.g. \"Kato Page 1\".\n"
